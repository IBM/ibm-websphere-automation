...
Create the CPFS pre-requisites application:

#### Example 1: Deploying the app with values overriden
Overrides to deploy the application in AllNamespaces mode
```
cat <<EOF | oc apply -f -
apiVersion: argoproj.io/v1alpha1
kind: Application
...
spec:
  ...
  source:
    repoURL: 'https://github.com/IBM/ibm-websphere-automation'
    path: gitops-deployment/cpfs-pre-requisites
    targetRevision: main
    helm:
      valueFiles:
        - values.yaml
