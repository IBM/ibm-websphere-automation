...
check_catalog_source() {
    ...
    if [[ "$cs_count" != "1" ]]; then
        echo "==> Error: The CatalogSource '${cs_name}' does not exist. Follow the instructions on https://www.ibm.com/docs/en/cloud-paks/foundational-services/4.9?topic=online-installing-foundational-services-by-using-console#catalog-sources to install all CatalogSources. Exiting."
        exit
    fi
}
