...
#   Optional parameters:
#       --websphere-automation-version $WSA_VERSION_NUMBER - the semantic version of WebSphere Automation operator (i.e. "1.7.3") that is targeted for upgrade.
...
#       --common-services-case-version $COMMON_SERVICES_CASE_VERSION - Case version of IBM Cloud Pak foundational services (Common Services) is installed. Defaults to 4.6.3.
...
main() {
    ...
    ./cp3pt0-deployment/setup_tenant.sh --license-accept --enable-licensing --operator-namespace ${WSA_OPERATOR_NAMESPACE} --source ${COMMON_SERVICES_CATALOG_SOURCE} -v 1
    if [ "$?" != "1" ]; then
