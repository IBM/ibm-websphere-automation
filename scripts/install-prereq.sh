...
#   It installs Network Policies for Foundational Services.
...
#   Optional parameters:
#       --websphere-automation-version $WSA_VERSION_NUMBER - the semantic version of WebSphere Automation operator (i.e. "1.8.2") that is targeted for installation.
...
check_args() {    
    ...
    if [[ -z "${CERT_MANAGER_NAMESPACE}" ]]; then
        ...
        echo "==> Cert Manager namespace is already set to: ${CERT_MANAGER_NAMESPACE}"
    fi
    ...
    if [[ -z "${LICENSING_SERVICE_NAMESPACE}" ]]; then
        ...
        echo "==> Licensing Service namespace is already set to: ${LICENSING_SERVICE_NAMESPACE}"
    fi
    ...
    if [[ -z "${COMMON_SERVICES_CASE_VERSION}" ]]; then
        ...
        echo "==> Common Services case version is already set to: ${COMMON_SERVICES_CASE_VERSION}"
    fi
...
main() {
    ...
    echo "==> Network Policies successfully created!"
    echo "      Install mode: ${INSTALL_MODE}"
    echo "      WebSphere Automation operator namespace: ${WSA_OPERATOR_NAMESPACE}"
    echo "      WebSphere Automation instance namespace: ${WSA_INSTANCE_NAMESPACE}"
}
